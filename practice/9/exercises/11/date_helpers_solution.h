...
/*
   Provide helper functions for the Date class, such as next_workay() and
   week_of_year().

Apologies: I had assumed the the formulas and explanations needed to make date
calculations
simple were more easily available on the web and clearer than I found.
The mathematical explanations seem to have been drowned in little widgets
that will
calculate for you, but don't explain how.

The next_workday() and week_of_year() are not very hard, but just about
anything further
gets you into trouble. I did a bit of that below.

To keep the code simple and comprehensible, the code below has been
restricted in
places (e.g. handling only dates after 1 Jan 1601).

Warning: Do not try anything non­trivial with date unless you are experienced with
either
data calculations, algorithms in general, or have a good source of algorithms (in
some
language) to guide your work.

This exercise was meant partly as a motivation for the next exercise: changing the
representation
of a date to "number of days after a designated first date" rather than
(day,month,year).
...
/*
   If you know the day of the week of *any* day (e.g. Jan 16, 2010 is a Saturday) and
   how to
   calculate leapyear() (exercise 9­10), you can find the day of the week of every day (of

   the
   current calendar). For example, 7 days after 1/16/2010 is another Saturday, 10 days
   after
   1/16/2010 is a "Saturday+3"; that is, a Tuesday. 10 days before 1/16/2010 is a
   "Saturday­3";
   that is, a Wednesday.
   ...
   So as not to try to invent everything from scratch, try typing "day of the week" into
   google (or equivalent).
   For more than you ever wanted to know about calendars, see
...
/*
   for simplicity of distribution on the web, I have copied the code of a Date class into this
   file.
   Had my aim been realism and proper code organization, I would have used a Date.h
   and a Date.cpp.
   */
...
namespace Chrono {
		...
		// e.g. Jan 1 is day #1, Jan 2 is day #1 and feb 3 is day #34
	{
		...
		if (Date::feb<a.month() && leapyear(a.year()))
			++m;                      // adjust if we passed a dat added for a leapyear
		return m+a.day();
	}
		...
		// days since default date
	{
		...
		/*
		   ...
		   365*y+31*m+d
		   However, ther real world (the real physical world + conventions) is not that
		   simple.
		   */
...
{
	cout << d << ": " << day_of_week(d) << "; linear: " << linear_day(d) << "; of week #" <<
		week_in_year(d) << '\n';
}
