...
/*
	...
	Declaration:
		"#" Name "=" Expression					// '#' will declare new variable
	...
	Primary:
		...
		"pow" "(" Expression "," Expression_integer ")"				// square root
...
const char let = '#';		// token representing the "let" keyword
...
const string quitkey = "exit";	// the exit keyword
const string sqroot = "sqrt";	// square root funtion function
const string power = "pow";	// power function
	...
	// parse pow ( Expression )
{
	...
	double index = 0;
	try {
		index = narrow_cast<int>(expression());
...
double primary()
{
	...
	case number:
		return t.value;
	case name:
		if (t.name == sqroot)			// sqrt function
			return parse_sqrt();
		if (t.name == power)
			return parse_pow();
