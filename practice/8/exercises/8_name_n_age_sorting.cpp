...
try
{
	...
	cout << "Enter names(x to stop):\n";
	for (string name; cin >> name;)			// get some names
		if (name != "x")
	...
	vector<int> ages(names.size());			// store their ages of each person
	cout << "Now ente their ages:\n";
	for (int i = 0, age = 0; i < ages.size(); ++i) {
		cin >> age;
		ages[i] = age;
	}
