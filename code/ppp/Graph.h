...
#include "Point.h"
//#include<vector>					// sam: commented --
//#include<string>
//#include<cmath>
#include "fltk.h"
#include "std_lib_facilities.h"				// sam: -- uncommented
...
struct Color {
	...
	Color(unsigned int cc) :c(Fl_Color(cc)), v(visible) { }			// sam: int -----> unsigned
...
public:
	...
	~Vector_ref() { for (unsigned i=0; i<owned.size(); ++i) delete owned[i]; }	// sam: int -----> unsigned
	...
	unsigned int size() const { return v.size(); }				// sam: int -----> unsigned
};
